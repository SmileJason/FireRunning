package com.weijie.firerunning.gps;
...
import com.weijie.firerunning.fragment.MapFragment;
...
public class LBSService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		filter.addAction("com.weijie.firerunning.gps.LBSService");
	...
	private class CommandReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (cmd == RunFragment.CMD_STOP_SERVICE) {// 如果发来的消息是停止服务
				flag = false;// 停止线程
				stopSelf();// 停止服务
			}
		...
		new Thread() {
			...
			public void run() {
				...
				while (flag) {
					...
					intent.setAction(MapFragment.LOCATION);
